...
export default class SubscriptionService {
    ...
    public static post = async (funcName: string, payload: any) => {
        try {
            const url = SUBSCRIPTION_SERVICE_BASE_URL + SUBSCRIPTION_ENDPOINT + WSDL;
            ...
            const response: any = await new Promise(async (resolve, reject) => {
                await soap.createClient(url, async (err: any, client: any) => {
                    ...
                    await client[funcName](payload, async (err: any, result: any) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(result);
            ...
            if (response === null || response === undefined) {
                throw new Error('SubscriptionService.post: response is null or undefined');
            }
